...
export class BookmarkService {
    ...
    async deleteBookmark(bookmarkId: string) {
        const bookmark = await this.prisma.bookmark.findFirst({where: {id: bookmarkId}});
        if (!bookmark) {
            return null;
        }
    ...
    async createBookmark(userId: string, publicationId: string) {
        const existingBookmark = await this.prisma.bookmark.findFirst({where: {userId: userId, publicationId: publicationId}});
        if (existingBookmark) {
